description 'This is a stack to create a VPC with two subnets for deploying HIPswitches and Endpoints'
...
# 4) 2 subnets, each in first availability zone
# 5) A default network route to the IGW
# 6) Associated plumbing resources to link it all together
# optional) 2 subnets, one in each of the account's first 2 availability zones

# Parameters may be defined using the `parameter` function
parameter :VpcName, default: 'Tempered VPC'
...
(1..3).each do |i|
  resource "subnet#{i}", 'AWS::EC2::Subnet' do
    ...
    # XXX - still having issues with the following construct
    # availability_zone Fn::select(i, Fn::get_azs(AWS::region))
    #availability_zone Fn::select(1, Fn::get_azs(AWS::region))
